...
struct TeamTab: View {
    ...
    var body: some View {
        let _ = print("[🟠 TEAMTAB] TeamTab.body rendering")
                            ...
                            if kickbaseManager.teamPlayers.isEmpty {
                                ...
                                List {
                                    ForEach(
                                        ...
                                        let _ = print(
                                            "[LIST] Rendering player \(index + 1): \(player.fullName)"
                                        )
    ...
    private var filteredAndSortedPlayers: [TeamPlayer] {
        ...
        print(
            "[⚡ FILTER] filteredAndSortedPlayers computed: \(filtered.count) players after filtering"
        )

        let sorted = filtered.sorted(by: { player1, player2 in
            switch sortBy {
            case .name:
...
struct TeamPlayerRow: View {
    ...
    var body: some View {
        let _ = print("[DEBUG] TeamPlayerRow.body rendering: \(teamPlayer.fullName)")
        Button(action: {
            ...
            HStack(spacing: 12) {
                ...
                let photoUrl: URL? = {
                    ...
                    if let imgString = ligaPlayer?.imageUrl, let url = URL(string: imgString) {
                        print(
                            "[TeamPlayerRow] USING LIGAINSIDER: \(teamPlayer.firstName) \(teamPlayer.lastName) -> \(imgString)"
                        )
                        return url
                    }
                    // Fall back to teamPlayer imageUrl
                    if let url = teamPlayer.imageUrl {
                        print(
                            "[TeamPlayerRow] USING KICKBASE: \(teamPlayer.firstName) \(teamPlayer.lastName) -> \(url)"
                        )
                        return url
                    }
                    print(
                        "[TeamPlayerRow] NO PHOTO: \(teamPlayer.firstName) \(teamPlayer.lastName)")
                    return nil
                }()

                let _ = print(
                    "[🔴 CRITICAL DEBUG] photoUrl result for \(teamPlayer.firstName) \(teamPlayer.lastName): \(photoUrl?.absoluteString ?? "NIL")"
                )

                if let url = photoUrl {
                    // Show player photo
                    let _ = print("[🟢 AsyncImage] Will show photo: \(url.absoluteString)")
                    #if !SKIP
                                ...
                                VStack(spacing: 2) {
                                    image
                                        ...
                                        .overlay(
                                            Circle()
                                                .stroke(
                                                    positionColor(teamPlayer.position), lineWidth: 2
                                                )
                                        )
...
struct TeamPlayerRowWithSale: View {
    ...
    var body: some View {
        let _ = print(
            "[DEBUG] TeamPlayerRowWithSale.body rendering: \(teamPlayer.fullName)"
        )

        // Calculate photoUrl HERE inside body with getLigainsiderPlayer() called INSIDE the closure
        let photoUrl: URL? = {
            print(
                "[🔵 PHOTOURL] Computing photoUrl for \(teamPlayer.firstName) \(teamPlayer.lastName)"
            )
                ...
                if let url = photoUrl {
                    ...
                    let _ = print("[🟢 AsyncImage] Will show photo: \(url.absoluteString)")
                        ...
                        AsyncImage(url: url) { phase in
                            switch phase {
                            case .empty, .failure:
                                // Loading or failure state - show position circle
                                VStack {
                                    Text(positionAbbreviation(teamPlayer.position))
                                        .font(.caption2)
                                        .fontWeight(.bold)
